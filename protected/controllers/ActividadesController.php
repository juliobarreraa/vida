...
/*		$dataProvider=new CActiveDataProvider('Actividad');
		...
		$now=date("Y-m-d");
		$row=Actividad::model()->find("DATE(fechaActivacion)='".date("Y-m-d")."'");
		if ($row==null)
			return;
		$respuesta=RespuestaActividad::model()->find('idActividad='.$row->idActividad." AND idUsuario=".Yii::app()->user->id);
		if ($respuesta==null)
		{
			$respuesta=new RespuestaActividad;
			$respuesta->idActividad=$row['idActividad'];
			$respuesta->idUsuario=Yii::app()->user->id;
			$respuesta->fechaIngreso=date('Y-m-d H:i:s');
			$respuesta->save(false);
		}
		if ($respuesta->respuesta=="")
			$this->render('actividad',array('model'=>$row,'respuesta'=>$respuesta));
		else
			$this->render('gracias');
	}
